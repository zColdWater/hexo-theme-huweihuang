...
AES CBC模式 PKCS5Padding
=======

日志例子: 需求是这样的，我每次写入一小段，每一小段都要aes加密，当数据大小到一定程度，就结束加密，给其他人可以一起解出来。

`C端:` 我拿初始的[key和iv]对我的每一次日志输入进行加密，当加密数据大于16字节就开始加密，没到16字节保存起来，加上下一次一起 如果超过16字节再加密，当文件到一定大小，我们结束这次加密，Padding填充上。

`S端:` S端拿到整个数据，直接拿开始的[key和iv]进行解密，就会得到原文了，因为aes加密，会每16字节用一个iv进行解密，当做完一个16字节，开始下一个的时候，iv会自动变，直到全部解密出来，所以和你加密过程一致。 
