...
当程序中开启多线程时，并发的用`写`操作同一个变量的时候，会出现线程不安全，例如 同时2个线程同时对变量A进行`写`操作，虽然程序里的执行顺序是操作A早于操作B，但是因为是多线程并发，实际执行的时候却是操作B早于操作A，这样你的逻辑就出错了，本来你想的是用操作B的值覆盖掉操作A，现在反过来了，甚至可能会Crash。  
...
### 清理机制
清理机制这里我就简单带过了 下面介绍几个`PINCache`里面的属性

1. Date: 设置一个过期时间，当缓存内容超过这个日期，会被自动清理掉。
2. Cost: 成本代价，未每个存储的Value设置一个Cost，当总的Cost>LimitCost的时候会去清理缓存内容来平衡。
3. ByteCount: 磁盘缓存的总字节大小，每次缓存的时候都会重新计算更新一次。
...
我们通过阅读`PINCache`的源码了解到核心的实现原理，如果你之前对缓存框架一头雾水，现在是不是清楚一些了呢。
